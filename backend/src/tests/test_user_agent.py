...
def test_addding_info_bit_to_memory():
    memory = UserAgent.Memory(mem_capacity=10)
    inf2 = Information()
    inf3 = Information()
...
def test_overflowing_memory():
    memory_capacity = 5
    memory = UserAgent.Memory(mem_capacity=memory_capacity)
    for i in range(10):
        inf = Information()
...
def test_calculating_mean_info_position():
    no_of_inf = 10
    positions = []
    memory = UserAgent.Memory(mem_capacity=no_of_inf)
...
def test_trying_to_integrate_info_second_time_fails(mocker):
    ...
    inf1 = Information()
    user.memory.add_new_info_bit(inf1)
